# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 14:49:56 2023
...
data = reframed [ contain_vars + ['var4(t)'] + [('var4(t+%d)' % (j)) for j in range(1,n_out)]]
...
inv_yhat=scaler.inverse_transform(np.reshape(yhat,(len(yhat),n_in*n_vars+n_out)))[:,3]
y = np.repeat(test_y,n_in*n_vars+n_out, axis=-1)
#inv_y=scaler.inverse_transform(y)[:,2]
inv_y=scaler.inverse_transform(np.reshape(y,(len(test_y),n_in*n_vars+n_out)))[:,3]
...
plt.plot(x, inv_y, marker='*',markersize='1',color='g',linewidth=1, label='True diff')
plt.plot(x, inv_yhat, marker='s',markersize='1', color='red',linestyle="--",linewidth=1,label='Predicting diff')

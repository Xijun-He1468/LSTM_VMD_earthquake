# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 13:51:32 2023
...
data10=dataset['longitude'].values
...
data=values
close_longitude = data[:,1] # 取出对应列
close_longitude.shape
...
tol = 1e-7     # 收敛精度 
u_longitude,u_hat_longitude,omega_longitude=VMD(close_longitude, alpha, tau, k, DC, init, tol)
...
for i in range(k):
    ...
    plt.subplot(k,1,i+1)
    plt.plot(u_longitude[i,:],color = 'black',linewidth=1)
    plt.xlabel("sample")
...
for imf in u_longitude:
    imf=imf.reshape(-1,1)
    imf=pd.DataFrame(imf)
    data_1=pd.DataFrame(data[:,(0,2,3)]) # 取出除对应列以外的其他列
...
plt.xticks(x[::50])
plt.tick_params(labelsize=10)  #改变刻度字体大小
plt.plot(x, close_longitude[1190-len(mean_pre_test):], marker='*',markersize='1',color='g',linewidth=1, label='True longitude')
plt.plot(x, mean_pre_test, marker='s',markersize='1', color='red',linestyle="--",linewidth=1,label='Predicting longitude')
plt.rcParams.update({'font.size': 10})  #改变图例里面的字体大小
plt.legend(loc='upper right')
plt.xlabel("Sample",fontsize=10)
plt.ylabel("Longitude",fontsize=10)
plt.legend()
# plt.savefig('新疆VMD-LSTM震级预测结果图.png')
plt.show()

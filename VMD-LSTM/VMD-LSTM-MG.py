# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 09:26:53 2023
...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 13:38:42 2023
...
data=values
close_magnitude = data[:,2] # 取出对应列
close_magnitude.shape
...
tol = 1e-7     # 收敛精度 
u_magnitude,u_hat_magnitude,omega_magnitude=VMD(close_magnitude, alpha, tau, k, DC, init, tol)
...
for i in range(k):
    ...
    plt.subplot(k,1,i+1)
    plt.plot(u_magnitude[i,:],color = 'black',linewidth=1)
    plt.xlabel("sample")
...
for imf in u_magnitude:
    imf=imf.reshape(-1,1)
    imf=pd.DataFrame(imf)
    data_1=pd.DataFrame(data[:,(0,1,3)]) # 取出除对应列以外的其他列
...
plt.xticks(x[::50])
plt.tick_params(labelsize=10)  #改变刻度字体大小
plt.plot(x, close_magnitude[1190-len(mean_pre_test):], marker='*',markersize='1',color='g',linewidth=1, label='True magnitude')
plt.plot(x, mean_pre_test, marker='s',markersize='1', color='red',linestyle="--",linewidth=1,label='Predicting magnitude')
plt.rcParams.update({'font.size': 10})  #改变图例里面的字体大小

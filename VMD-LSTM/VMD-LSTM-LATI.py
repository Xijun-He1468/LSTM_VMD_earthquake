...
data10=dataset['latitude'].values
...
data=values
close_latitude = data[:,0] # 取出对应列
close_latitude.shape
...
tol = 1e-7     # 收敛精度 
u_latitude,u_hat_latitude,omega_latitude=VMD(close_latitude, alpha, tau, k, DC, init, tol)
...
for i in range(k):
    ...
    plt.subplot(k,1,i+1)
    plt.plot(u_latitude[i,:],color = 'black',linewidth=1)
    plt.xlabel("sample")
...
for imf in u_latitude:
    imf=imf.reshape(-1,1)
    imf=pd.DataFrame(imf)
    data_1=pd.DataFrame(data[:,(1,2,3)]) # 取出除对应列以外的其他列
...
plt.xticks(x[::50])
plt.tick_params(labelsize=10)  #改变刻度字体大小
plt.plot(x, close_latitude[1190-len(mean_pre_test):], marker='*',markersize='1',color='g',linewidth=1, label='True latitude')
plt.plot(x, mean_pre_test, marker='s',markersize='1', color='red',linestyle="--",linewidth=1,label='Predicting latitude')
plt.rcParams.update({'font.size': 10})  #改变图例里面的字体大小
